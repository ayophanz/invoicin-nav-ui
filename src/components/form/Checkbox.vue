<template>
    <div class="mb-2 chekcbox-component">
        <div :class="errorMessage !== '' ? 'border-red-500' : 'border-transparent'" class="border px-2 rounded-md flex h-6 items-center">
            <input 
                :id="name" 
                :name="name" 
                v-model="checkbox"
            ...
            <label v-if="label !== ''" :for="name" class="pl-3 cursor-pointer text-sm font-medium text-gray-900">{{ label }}</label>
        </div>
